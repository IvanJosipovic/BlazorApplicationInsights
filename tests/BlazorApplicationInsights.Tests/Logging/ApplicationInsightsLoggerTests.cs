...
//            logger.LogInformation(MessageSimple);
//            _appInsightsMock.Verify(x => x.TrackTrace(new TraceTelemetry() { Message = MessageSimple, SeverityLevel = SeverityLevel.Information }, It.IsAny<Dictionary<string, object?>?>()), Times.Once);
//            _appInsightsMock.VerifyNoOtherCalls();
